<div class="payment-slide">
                <ul>
                   <li  onclick="window.location='../index.php'"><i class="fas fa-external-link-alt"></i> Home page</li>
                  <li onclick="window.location='../controller/orderCon.php?id=1'"><i  class="fas fa-hourglass-half"></i> Pending Orders</li>
                  <li onclick="window.location='paymentFood_accept.php'"><i  class="fas fa-clipboard-check"></i> Accepted Orders</li>
                  <li onclick="window.location='paymentFood_receving.php'"><i  class="fas fa-truck"></i> Receiving Order</li>
                  <li onclick="window.location='paymentFood_history.php'"><i class="fas fa-history"></i> Order History</li>
                  <li onclick="window.location='foodposts.php'"><i  class="fas fa-plus"></i> New Order</li>
                  <div class="taggle-box">
                      <h5>You have to issue get order or deliver order? You can disable the request some time period.</h5>
                    <div class="taggle">
                        <h4>Order : <span style="color: chartreuse;">Available</span></h4>
                        <form action="../controller/orderCon.php?avail" method="post">
                            <input onchange="this.form.submit();" type="checkbox" name="isAvail" value="true"  checked>
                            <label for="" class="onbtn">On</label>
                            <label for="" class="ofbtn">Off</label>
                        </form>
                    </div>
                  </div>
                  <div class="line"></div>
                  <h3 class="line1">OR</h3>
                  
                  <div class="select-box">
                  <h5>Select date disable order request</h5>
                     <div class="date-block">
                        <input id="1" type="date">
                     </div>
                      </form>
                      <h6>@Bodima</h6>
                  </div>
                  
              </ul>
          </div> 